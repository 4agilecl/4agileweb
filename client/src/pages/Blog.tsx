            ...
            {articles.map((article) => (
              <Link href={`/blog/${article.slug}`} key={article.slug}>
                <article className="primary-service-card" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', padding: 0 }}>
                  <img src={article.image} alt={article.title} style={{ width: '100%', height: '240px', objectFit: 'cover', borderBottom: '1px solid var(--line)' }} />
                  <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ fontSize: '24px', marginBottom: '16px' }}>{article.title}</h3>
                    <p style={{ flex: 1, color: 'var(--muted)' }}>{article.excerpt}</p>
